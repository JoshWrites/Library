...
| ivrit-ai whisper-large-v3-turbo-ct2 | STT only | CTranslate2 | n/a (CPU only on RDNA1; ROCm fork targets RDNA3) | No usable path | int8 ~1.7 GB / fp16 ~3.2 GB | **Best Hebrew (Apache-2.0, trained on 22k+ hr)** | Yes (Whisper base) | Apache-2.0 | CPU on Ryzen 5950X ≈ 1–2× RT |
...
1. **whisper.cpp Vulkan** — mature, well-tested. The current open issue
   ...
   pin a patched build.
2. **CPU** — Ryzen 9 5950X handles ivrit-ct2 turbo at int8 in ~1–2× RT,
